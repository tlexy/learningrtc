...
bool JitterBufferEntity::force_cache()
{
	return false;
}

void JitterBufferEntity::update()
{
	bool flag = false;
	if (force_cache())
	{
...
void JitterBufferEntity::on_rtp_packet(rtp_packet_t* rtp)
{
	_mutex.lock();
...
void JitterBufferEntity::set_output_buffer(int64_t ms)
{
	_output_ms = ms;
}

void JitterBufferEntity::decode()
{
	do_decode();
}
...
int AacJitterBufferEntity::get_pcm_buffer(int8_t* data, int len)
{
	if (!_output_init)
	{
		return 0;
...
void AacJitterBufferEntity::aac_init()
{
	//尝试获取音频并解析音频参数
...
void AacJitterBufferEntity::do_decode()
{
	if (!_is_init)
	{
		aac_init();
